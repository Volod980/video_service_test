...
# Health check — start-period gives uvicorn time to boot before first probe
HEALTHCHECK --interval=30s --timeout=10s --start-period=30s --retries=3 \
    CMD python -c "import urllib.request; urllib.request.urlopen('http://localhost:8000/health')" || exit 1

# Single worker: in-memory JOBS dict is not safe across multiple processes.
# Use ${PORT:-8000} so Railway's injected PORT is respected.
